...
type TrainingEditionInfoProps = {
  headingId: string;
  result: ContentResult<TrainingEdition | null>;
};
...
export default function TrainingEditionInfo({
  ...
  const edition = result.status === "available" ? result.data : null;
          ...
          <div>
            <dl className="grid gap-x-10 gap-y-6 sm:grid-cols-2">
              {edition.editionLabel ? (
                <EditionDetail label="Edición">{edition.editionLabel}</EditionDetail>
              ) : null}
              {edition.year ? (
...
function getStateMessage(
  result: Exclude<
    ContentResult<TrainingEdition | null>,
